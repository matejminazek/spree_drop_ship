  ...
  context 'as supplier user' do
    context 'given a product with a variant and a stock location' do
      before do
        ...
        option_value = create(:option_value, option_type: create(:option_type))
        @v = @product.variants.create!(sku: 'FOOBAR', option_values: [option_value])
      ...
      context 'with single variant' do
        ...
        it 'can create a new stock movement', js: true, driver: :selenium do
        ...
        it 'can create a new negative stock movement', js: true, driver: :selenium do
          select2 @user.supplier.name, from: 'Stock Location'
          fill_in 'stock_movement_quantity', with: -5
      ...
      context 'with multiple variants' do
        before do
          option_value = create(:option_value, option_type: create(:option_type))
          v = @product.variants.create!(sku: 'SPREEC', option_values: [option_value])
          @product.add_supplier! @user.supplier
    ...
    context 'with no stock location' do
      before do
        ...
        @product.add_supplier! @user.supplier
        option_value = create(:option_value, option_type: create(:option_type))
        @product.variants.create!(sku: 'FOOBAR', option_values: [option_value])
